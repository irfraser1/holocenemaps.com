...
Collection JavaScript is still primarily inline in `collection.html`, with low-risk shared helpers loaded from `js/collection-ui-helpers.js` and map photo-management behavior loaded from `js/collection-photo-manager.js`. These files are classic browser scripts so existing global function lookup and inline event handlers keep working without a build step.
...
6. `js/collection-photo-manager.js` after UI helpers and before the main app script.
7. Existing inline JavaScript in its current order.

The CSS file order is part of the contract. Later files may rely on variables and reset rules from `collection-base.css`.
The helper and photo-manager JS files must load before the main inline app script because the app calls those helpers by global function name. The photo manager depends on shared globals initialized by the main app at runtime, including `db`, `user`, `maps`, `editingId`, `_detailMapId`, `loadMaps`, `toggleCard`, `renderList`, and dialog helpers.
...
3. Extract rendering functions for map cards, details, dialogs, and chat into separate modules.
